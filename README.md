...
Walkthrough mode uses chunk-level sync instead of exact line sync. Clicking a line in the source editor highlights the corresponding explanation paragraph block in the right panel with a blue background and scrolls it into view; scrolling the source editor also tracks the matching chunk. Inline hints and per-line active-row highlighting are disabled. Standalone comments and docstrings are skipped as separate explanation topics; they are used only as context when explaining the executable code that follows.
